<?php
/*
Plugin Name: Ligue 1 Football Rankings Lite
...
function LIGUE1PRLPageURL() {
 $LIGUE1PRLpageURL = 'http';
 $LIGUE1PRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $LIGUE1PRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $LIGUE1PRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $LIGUE1PRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_ligue1sprlstats() 
{
?>
<h2>Ligue 1 Rankings Lite</h2>
<?php ligue1sprl_stats(); ?>
<?php
}

function ligue1sprlstats_install()
{
register_sidebar_widget(__('Ligue 1 Rankings Lite'), 'widget_ligue1sprlstats'); 
}
add_action("plugins_loaded", "ligue1sprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'ligue1sprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'ligue1sprl_stats_remove' );

function ligue1sprl_stats_install() 
{
...
add_option("ligue1sprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("ligue1sprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("ligue1sprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE LIGUE 1 Power Rankings Installation", "Hi\n\nLITE LIGUE 1 Power Rankings Activated at \n\n".LIGUE1PRLPageURL()."\n\nLIGUE 1 Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function ligue1sprl_stats_remove() 
{
/* Deletes the database field */
delete_option('ligue1sprl_scroll_text_color');
delete_option('ligue1sprl_scroll_text_color1');
delete_option('ligue1sprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'ligue1sprl_stats_admin_menu');

function ligue1sprl_stats_admin_menu() {

add_options_page('Ligue 1 Rankings Lite', 'Ligue 1 Rankings Lite Settings', 'administrator', 'ligue-1-rankings-lite.php', 'ligue1sprl_stats_plugin_page');
}
}
function ligue1sprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('ligue1sprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('ligue1sprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('ligue1sprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/ligue_1_stats_rankings_magpie_lite/ligue_1_stats_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = ligue1sprl_fetch_rss( $url );
...
function ligue1sprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>Ligue 1 Rankings Options Page</h2>
...
<p>
<?php echo ligue1sprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete LIGUE 1 Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/ligue-1-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/ligue-1-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
...
<p><strong>Soccer / Football / Futbol</strong><br />
  <strong>Premier League rankings</strong> <a href="http://wplatest5.thoseamazingparks.com">Preview</a> | <a href="http://www.ibet.ws/download/ligue-1-rankings.zip">Download Now</a> - Complete Premier League Rankings for all 20 Teams.</p>
